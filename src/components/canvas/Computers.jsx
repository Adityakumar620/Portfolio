...
const Computers = ({ isMobile }) => {
  ...
  return (
    <mesh>
      ...
      <primitive
        object={computer.scene}
        scale={isMobile ? 0.65 : 1.15} // Increased scale for better visibility
        position={isMobile ? [0, -3.5, -2.2] : [0, -4, -1.5]} // Adjusted position
        rotation={[-0.01, -0.2, -0.1]}
      />
    </mesh>
  );
...
const ComputersCanvas = () => {
  ...
  return isWebGLSupported ? (
    <Canvas
      ...
      camera={{ position: [20, 3, 5], fov: 25 }}
      gl={{ preserveDrawingBuffer: true }}
      style={{ height: isMobile ? "50vh" : "60vh", width: "100%" }} // Increased height
    >
      <Suspense fallback={<CanvasLoader />}>
        <OrbitControls
          enableZoom={false}
